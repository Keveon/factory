...
import com.keveon.repository.NewsRepository;
import com.keveon.service.NewsService;
import com.keveon.utils.Clock;
        ...
        return repository.findAll((root, query, cb) -> {
            ...
            if (hasText(keywords)) {
                ...
                for (String s : keywords.split(","))
                    if (hasText(s)) tempPredicates.add(cb.like(root.get("title").as(String.class), "%" + s + "%"));

                predicates.add(cb.or(tempPredicates.toArray(new Predicate[tempPredicates.size()])));
            }
